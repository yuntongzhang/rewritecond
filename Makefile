...
.PHONY: loop_convert
loop_convert: $(BUILDDIR)/loop_convert

.PHONY: rewrite_cond
rewrite_cond: $(BUILDDIR)/rewrite_cond

$(BUILDDIR)/loop_convert: LoopConvert.cpp
	$(CXX) -v $(CXXFLAGS) $(LLVM_CXXFLAGS) $(CLANG_INCLUDES) $^ $(CLANG_LIBS) $(LLVM_LDFLAGS) -o $@
